...
// Realiza un movimiento. Devuelve 1 si el movimiento es legal, sino 0 (incluye hacer movimiento nulo)
int		MakeMove(MOVE move)		{
	...
	// No estar en jaque despues de hacer movimiento. No es necesario con movimiento nulo (se chequea antes)
...
// Permite conocer el tipo de movimiento que llega. Pasa por referencia la posición del ultimo carácter analizado de ´c´
...
// Dado un movimiento(formato SAN) determina si es legal o no. Retorna el movimiento(si es legal) o vacío(en otro caso).
