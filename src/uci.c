...
void	ParsePositionCommand()	{
	...
	/*
	 * La GUI podría enviar también con el comando fen una posicion que no es la inicial del ajedrez normal y 
	 * despues una lista de movimientos, en este caso en cada movimiento el motor configurara la posicion fen 
	 * enviada por la GUI
	 */
...
void	ParseGoCommand()		{
	...
	MOVE	best_move	= EMPTY_MOVE;
	int		time[3]		= {0, 0, 0,},							// almacena tiempos de cada bando
			inc[3]		= {0, 0, 0,},							// almacena incrementos de cada bando
			max_depth[3]= {MAX_DEPHT, MAX_DEPHT, MAX_DEPHT,},	// almacena la maxima profundidad de cada bando
			mat_sum;

	RemainingMove		= DEFAULT_REMAINING_MOVE;				// si no hay número de movimientos para el control imaginamos que quedan 40
	Mps					= DEFAULT_MPS;
	...
	while((Opc = MyStrtok(NULL, " ", &SavePointer)) != NULL)	{
		...
		// numero de movimentos que faltan para el control de tiempo
			...
			Mps				= atoi(MyStrtok(NULL, " ", &SavePointer));
		...
		// modo tiempo fijo por movimiento
			...
			TimeTotal		= atoi(MyStrtok(NULL, " ", &SavePointer));
			max_depth[Turn]	= MAX_DEPHT;
		}
	}


	// Recuperando informacion de la configuracion para la busqueda
	TimeTotal	= time[Turn];
	Inc			= inc[Turn];
	MaxDepth	= max_depth[Turn];
	...
	// Calcula tiempo para pensar si es necesario y busca el mejor movimiento
		...
		CalcTimeForMove(TimeTotal, OutOfBook, mat_sum / 100);
		EngineThink(SEARCHING);
	}
