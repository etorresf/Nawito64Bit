...
int		Extensions(int PV_node, int check_mate_threat, int negative_SEE)		{
	...
	if (Board[to] < KNIGHT)		{
		BITBOARD	bb	= RankBB[to + Front[Turn ^ 1]];

		if (bb & (RANK_1_BB | RANK_8_BB))
			return HALF_PLY	<< PV_node;

		if (bb & (RANK_2_BB | RANK_7_BB))
			return (HALF_PLY / 2)	<< PV_node;
	}
...
STATE	StopSearch(int score)	{
	...
	switch (Engine & (SEARCHING | ANALYZING))		{
		case SEARCHING:
			if (Engine & ABORT)	{
				time_used	= ClockTime() - TimeLeft;

				// tiempo limite sobrepasado, salir de la busqueda 
				if (time_used >= TimeLimit)
					return	Engine	|= STOP;

				// tiempo para buscar nuevo movimiento agotado,salir de la busqueda si evaluacion del actual PLY no se ha reducido en un MARGEN
				if (time_used > NoNewMov && score + MARGEN >= Value1[Li - 1])
					return	Engine	|= STOP;
...
int		QuiesenceSearch(int alpha, int beta, int depth)			{
	...
	Nodes++;
	if (!(Nodes % Cknodes) && StopSearch(score))
		return 0;
	...
	if (!in_check)	{
		...
		// delta prunning. Entre 200 en mediojuego y 500 en final
		if (GAME_PHASE > 85)
			delta_prunning		= alpha - (650 - (450 * GAME_PHASE) / 256) - static_eval;
	}
...
int		PVS(int alpha, int beta, int depth, int make_null_move)	{
	...
	if (!(in_check || PV_node) && alpha > -MATE && beta < MATE_SCORE)			{
		...
		// Razoring
		if (!hash_move && (depth <= RP) && (eval_score + RAZORING_MARGEN(depth) <= alpha))		{
			if (depth <= PLY)
		...
		if (PieceMat[Turn])		{
			...
			if (depth < RFP && (eval_score >= beta + REVERSE_FUTILITY_MARGEN(depth)))
				return eval_score;


			// Null Move Reduction
			depth_null	= depth - (PLY + R + depth / 6.0);
		...
		if (depth < PLY4 && alpha > -MATE_SCORE)
			can_prune_FP = eval_score + Fmargen[depth / PLY] < beta;
	}
...
void	PrintPV(int depth, int score)			{
	int		i, 
			time_used	= ClockTime() - TimeLeft;				// Calcula el tiempo usado en milisegundos
	MOVE*	move		= PV;
	double	nps			= (time_used > 16) ? ((1000.0 * (double)Nodes) / (double)time_used) : 0;
	float	used_hash	= (1000.0 * (float)UsedHash) / (float)TTsize;


	// Envia profundidad, puntos, tiempo, nodos y nodos / segundos
	printf("info depth %d time %d nodes %d nps %d hashfull %i ", depth, time_used, Nodes, (int)nps, (int)used_hash);
...
int		PVSroot(int alpha, int beta, int depth)	{
	...
	switch (legal_moves)		{
		// Mate o ahogado
		case 0:
			best_score	= InCheckRoot ? (Ply - MATE) : DRAW_VALUE(Turn);
			break;
...
void	EngineThink(int state)	{
	...
	Cknodes			= 60 * GAME_PHASE + 1024;	// Cada cuanto "nodos" revisar tiempo? Entre 16384 en mediojuego y 1024 en final
	...
	for (i = 1; i <= MaxDepth && (fail_count < 16) && !(Engine & STOP);)		{
		...
		if ((Engine & SEARCHING) && i > PLY)	{				// Si la cosa es clara, reducir el tiempo de búsqueda
			Engine |= ABORT;									// Se tiene una profundidad máxima para abandonar la búsqueda

			if (time_used > NoNewIteration)
