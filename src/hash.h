...
// Tipos de movimientos (de TTables), de momento solo se usan 4 bits
typedef	enum	{
	HASF_NULL		= 0<<0,
	HASF_EXACT		= 1<<0,
	HASF_ALPHA		= 1<<1,
...
// Evaluación de cache: guarda las posiciones previamente analizadas de la estructura de PEONES. incluye la posicion y PEONES alfrente de los REYES
typedef struct	{
	HASH_KEY	Hash,
				PawnsPassed;
	I16			MidScore,
...
void			StorePawnScore(int mid_score, int end_score, int black_casttle, int white_casttle, HASH_KEY key, HASH_KEY pawns_passed);
int				ProbePawnScore(int* mid_score, int* end_score, int* black_casttle, int* white_casttle, HASH_KEY key, HASH_KEY* pawns_passed);
